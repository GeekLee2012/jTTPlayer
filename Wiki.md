...
## 四、歌词页
#### 跳转播放到指定歌词行
- Windows：双击想要播放的歌词行即可，`指示线`仅为兼容其他平台、显示时间等用途
- macOS: 滚动歌词，将想要播放的歌词行滚动到`指示线`位置即可

## 五、主菜单
#### 查看
- 打开/关闭界面：播放列表、均衡器、歌词秀、音乐窗、迷你模式等
- 关闭`迷你模式`：部分皮肤在切换至迷你模式后，无`返回正常模式`按钮。 点击`主菜单 - 查看 - 迷你模式`，可返回正常模式
...
## 七、常见问题 FAQ
- 播放器`无法启动`、或播放器`界面异常且无法操作`
> 答曰：重置配置（删除`default.cfg`配置文件），将`千千选项`全部设置恢复为刚安装完的默认状态。   
各平台数据目录位置：  
macOS: / Users / 用户名 / Library / Application Support / jTTPlayer  
Windows: C: / Users / 用户名 / AppData / Roaming / jTTPlayer  
Linux: / Users / 用户名 / Documents / jTTPlayer  
当然，直接`删除 jTTPlayer 目录`也可以，不过会导致`播放列表`也被`全部删除`。

- 播放器为什么`不支持原版千千静听皮肤`
> 答曰：技术栈缺陷导致。皮肤文件主要涉及XML文件编码、图片格式等问题。  
而现有的JavaFX/Java技术栈，无法很好解决此类问题。  
因此，相比直接支持原版皮肤，移植皮肤更简单可靠。
  
- 播放器在切换播放内核后，部分界面`状态没有同步更新`
> 答曰：重启播放器即可

- 切换至`迷你模式`后，没有`退出迷你模式`按钮，无法返回`正常模式`
> 答曰：右键唤出主菜单，点击`主菜单 - 查看 - 迷你模式`，即可返回`正常模式`
